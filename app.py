...
with tab_winkel:
    ...
    # Verbinding met DB — laat app niet crashen bij tijdelijk verbindingsprobleem
    try:
        ...
        st.error(
            "⚠️ Kan geen verbinding maken met de database. "
            "Ververs de pagina en probeer het opnieuw. "
            "Als het probleem aanhoudt, neem contact op met Wouter."
        )
        winkels = []
        winkel_namen = []
        db_ok = False
    ...
    if st.button("Inloggen", type="primary", use_container_width=True):
        if not db_ok:
            st.error("Geen verbinding met de database. Ververs de pagina.")
        elif keuze == "— selecteer —":
